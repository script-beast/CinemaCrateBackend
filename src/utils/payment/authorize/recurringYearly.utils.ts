...
const recurringYearly = (cardDetails, product, user) => {
  ...
  orderType.setDescription(`Yearly Subscription for ${product.name}`);
  ...
  arbSubscription.setAmount(parseFloat(product.yearlyPrice).toFixed(2));
...
module.exports = recurringYearly;
